...
import { deleteCurso, hasCursoActiveLoans } from '@/services/cursos'
...
export function CursoDetail({ curso, open, onOpenChange, onEdit }: CursoDetailProps) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const hasActive = await hasCursoActiveLoans(curso.id)
      if (hasActive) {
        toast.error('Este curso possui livros emprestados e não pode ser excluído neste momento.')
        setDeleteOpen(false)
        return
      }
